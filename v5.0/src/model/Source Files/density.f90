...
REAL(R8) FUNCTION DENSITY (T,TDS,SS)
  ...
  IMPLICIT NONE
  REAL(R8) :: T,TDS,SS
  IF(T <= 0.0) THEN    ! Suggested by Stewart Rounds
                             DENSITY = 0.842594D0
  IF (SUSP_SOLIDS)     DENSITY = DENSITY+6.2D-4*SS
  IF (FRESH_WATER(JW)) DENSITY = DENSITY+TDS*8.221D-4
  IF (SALT_WATER(JW))  DENSITY = DENSITY+TDS*0.824493D0                      &
                                 +(-5.72466D-3)*TDS**1.5D0+4.8314D-4*TDS*TDS

...
! Hydraulic structures
  ...
  ! port area calculations  ! cb 3/6/23
  DO jb=1,nbr
    DO JS=1,NST
      IF (port_shape_type(JS,JB) == 2) THEN
        aport(js,jb) = aport_coef1(JS,JB)*aport_coef2(JS,JB)
      ELSE
        aport(js,jb) = (aport_coef1(JS,JB)/2.0)**2 * pi
      END IF
    END DO
  END DO  

        ...
        ! Maybe branches with downstream flow and external head boundaries don't get inactivated because JB=JBDN(JW). !SR 11/30/2021
          ...
          ! DO I=DS(JB)-1,DS(JB)                                                                                      !SR 11/30/2021
            DO I=CUS(JB)-1,DS(JB)                 ! do this after H1 and BH1 are updated for all relevant segs above  !SR 11/30/2021
              DO K=KT,KB(I)                                                                                           !SR 11/30/2021
                AVHR(K,I) =  H1(K,I)+(H1(K,I+1) -H1(K,I)) /(0.5*(DLX(I)+DLX(I+1)))*0.5*DLX(I)                         !SR 11/30/2021
                BHR1(K,I) = BH1(K,I)+(BH1(K,I+1)-BH1(K,I))/(0.5*(DLX(I)+DLX(I+1)))*0.5*DLX(I)                         !SR 11/30/2021
...
!****** Additional layer subtractions
      ...
      END DO
    END DO
!** Update derived concentrations if needed, to ensure they are updated prior to output                               !SR 11/10/2023
    !IF (DERIVED_CALC .AND. (ANY(LAYERCHANGE) .OR. CELLS_ACTIVATED)) CALL DERIVED_CONSTITUENTS                         !SR 11/10/2023    

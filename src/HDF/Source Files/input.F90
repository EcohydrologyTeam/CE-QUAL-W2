...
USE GLOBAL; USE NAMESC; USE GEOMC;  USE LOGICC; USE PREC;  USE SURFHE;  USE KINETIC; USE SHADEC; USE EDDY
  ...
  USE modSYSTDG        
  USE ALGAE_TOXINS; USE MetFileRegion, ONLY: NMetFileRegions, Met_Regions  ! SW 12/13/2023
  IMPLICIT NONE
...
! Title and array dimensions
  ...
  READ (CON,'(//8X,I8,8A8,f8.0,2A8)')  NOD,SELECTC,HABTATC,ENVIRPC,AERATEC,inituwl,ORGCC,SED_DIAG,DZMAX,HGMOD, HDF5C 
  ELSE
  READ (CON,*)
  READ (CON,*)
  READ (CON,*)
  ...
  READ (CON,*)
  READ (CON,*)
  READ (CON,*) NOD,SELECTC,HABTATC,ENVIRPC,AERATEC,INITUWL,ORGCC,SED_DIAG,DZMAX,HGMOD,HDF5C
  SELECTC=ADJUSTR(SELECTC);HABTATC=ADJUSTR(HABTATC);ENVIRPC=ADJUSTR(ENVIRPC);AERATEC=ADJUSTR(AERATEC);INITUWL=ADJUSTR(INITUWL)
  ORGCC = ADJUSTR(ORGCC); SED_DIAG=ADJUSTR(SED_DIAG); HGMOD=ADJUSTR(HgMOD); HDF5C=ADJUSTR(HDF5C)
  ENDIF
...
! Constituent, tributary, and widthdrawal totals
  ...
  ALLOCATE (C2CH(NCT),   CDCH(NDC),   EPCH(NEPT),  macch(nmct), KFCH(NFL), APCH(NAL), ANCH(NAL), ALCH(NAL), ENCH(NEPT), ELCH(NEPT))
...
!  ALLOCATE (HYDRO_PLOT(NHY),    CONSTITUENT_PLOT(NCT), DERIVED_PLOT(NDC))
  ...
  ALLOCATE (ADX(KMX,IMX),    ADZ(KMX,IMX),    DO1(KMX,IMX),    DO2(KMX,IMX),    DO3(KMX,IMX),    DO4(KMX,IMX),    SED(KMX,IMX))
  ...
  !
  ALLOCATE (theta_STRT(NSTT,NBR), wdz_multi(nbr), wdz_multi_read(nbr), wet_well(nbr), wet_well_read(nbr))       ! cb 10/29/22
  ALLOCATE (aport(nstt,nbr), aport_coef1(nstt,nbr), aport_coef2(nstt,nbr), ploss(nstt,nbr), str_flow_prof_eqn(nstt,nbr) )
  ALLOCATE (port_shape_type(nstt, nbr))
  ALLOCATE (vnorms(kmx,nstt), veljs(kmx,nstt), vsums(nstt), hswts(nstt), hswbs(nstt), elstrs(nstt), elmvs(nstt))
  ALLOCATE (kmvs(nstt), kstrs(nstt), ktops(nstt), kbots(nstt), jsdn(nstt), jsup(nstt), jspairup(nstt), jspairdn(nstt))
  ALLOCATE (portup(nstt), portdn(nstt), jsorder(nstt), estrorder(nstt), hloss(nstt))
  !
...
! State variables
  ...
  IF (HG_CALC) THEN
    Hg0   => C2(:,:,NHg0);   HgII   => C2(:,:,NHgII);   MeHg   => C2(:,:,NMeHg)
    Hg0SS => CSSK(:,:,NHg0); HgIISS => CSSK(:,:,NHgII); MeHgSS => CSSK(:,:,NMeHg)
  END IF  
...
! Inflow-outflow cards
  ...
  END DO
  DO JB=1,NBR            ! cb 4/4/23
    READ (CON,'(:8X,9F8.0)')          (theta_STRT(JS,JB), JS=1,NSTR(JB))
  END DO  
  ...
  END DO
  !
  DO JS=1,NSTT          ! cb 4/4/23
    READ (con,*)       (theta_STRT(JS,JB),  JB=1,NBR)
  END DO  
  READ (CON,*)
  READ (CON,*)
...
! Kinetics coefficients
  ...
  IF (ORGC_CALC) THEN
    READ (CON,'(//(8X,12F8.0))')      (LDOMCDK(JW), RDOMCDK(JW), LRDOMCDK(JW), LDOMPDK(JW), RDOMPDK(JW), LRDOMPDK(JW),             &
                                       LDOMNDK(JW), RDOMNDK(JW), LRDOMNDK(JW),                                 JW=1,NWB)
  ELSE
  ...
  END IF
  IF (ORGC_CALC) THEN
    READ (CON,'(//(8X,15F8.0))')     (LPOMCDK(JW), RPOMCDK(JW), LRPOMCDK(JW), POMS(JW), LPOMHK(JW),  RPOMHK(JW), LPOMPDK(JW),      & 
                                      RPOMPDK(JW), LRPOMPDK(JW), LPOMNDK(JW), RPOMNDK(JW), LRPOMNDK(JW),       JW=1,NWB)
  ELSE
  ...
  READ (CON,'(//(8X,2F8.0))')         (KDO(JW),    ANOX(JW),                                                   JW=1,NWB)
...
! Kinetics coefficients
  ...
  IF (ORGC_CALC) THEN
    ...
    !ZZ 9/2023: 8 new parameters for POM card
  ...
  READ (CON,*)          (KDO(JW),     JW=1,NWB)
  READ (CON,*)          (ANOX(JW),    JW=1,NWB)
...
!  Amaila start - reading additional sediment compartments coefficients
  ...
  ! Single wet well withdrawal input file  ! cb 8/19/22
  wdz_multi=.true.  ! this logical variable directs code to new withdrawal algorithm in hydroinout subroutine
  wet_well=.false.
  single_wet_well=.false.
  INQUIRE(FILE='w2_singlewetwellwd.csv',EXIST=single_wet_well)
  IF(single_wet_well)THEN
    nunit=nunit+1
    OPEN(nunit,file='w2_singlewetwellwd.csv',status='old')
    READ (nunit,*)
    READ (nunit,*)
    READ (nunit,*)AID, (wet_well_read(JB),      JB=1,NBR)
    DO Jb=1,nbr
      IF (wet_well_read(jb)=="ON") wet_well(jb)=.true.
    END DO
    ! Shape of intake ports
    READ (nunit,*)
    DO JS=1,NSTT
      READ (nunit,*)AID,        (port_shape_type(JS,JB),  JB=1,NBR)
    END DO
    ! Cross-sectional area coefficients
    READ (nunit,*)
    DO JS=1,NSTT
      READ (nunit,*)AID,        (aport_coef1(JS,JB),  JB=1,NBR)
    END DO
    READ (nunit,*)
    DO JS=1,NSTT
      READ (nunit,*)AID,        (aport_coef2(JS,JB),  JB=1,NBR)
    END DO
    ! Intake port loss coefficients 
    READ (nunit,*)
    DO JS=1,NSTT
      READ (nunit,*)AID,        (ploss(JS,JB),  JB=1,NBR)
    END DO
    !
    READ (nunit,*)
    DO JS=1,NSTT
      READ (nunit,*)AID,        (str_flow_prof_eqn(JS,JB),  JB=1,NBR)
    END DO
    CLOSE (NUNIT)
  ENDIF  
...
! Output file unit numbers
  ...
  ALLOCATE (TSR(NIKTSR))
  ALLOCATE (WDO(NIWDO,5),WDO2(NWD+NST+NGT+NSP+NPU+NPI,4))
  !
  ! HgModule
  IF (HG_CALC) THEN
    ALLOCATE(TSR_Hg(NIKTSR)) 
    ALLOCATE(TSR_Hg2(NIKTSR))
  END IF
  !
  ...
  IF (HG_CALC) THEN
    DO J=1,NIKTSR
      TSR_Hg(J)  = NUNIT; NUNIT = NUNIT+1
      TSR_Hg2(J) = NUNIT; NUNIT = NUNIT+1
    END DO
  END IF 

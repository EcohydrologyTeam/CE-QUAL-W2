...
! IVF/CVF specific code
 ...
 !DEC$ATTRIBUTES REFERENCE :: Dlg
  ...
  USE modSYSTDG, ONLY: INPUT_SYSTDG                               
  Use CEMAVars
  USE CEMASedimentDiagenesis, only: C2SF,InitCond_SedFlux
  USE HgModule, ONLY: HgModule_Input, BedISS, Hg2, HgKineticFluxesOutputInit, JDAYSTARTHG, NXTMFLHG, HGFLUXINT, HgModule_IC
  USE INITIALVELOCITY; USE ENVIRPMOD
  USE BIOENERGETICS
  USE BUILDVERSION; USE MetFileRegion
  IMPLICIT NONE
...
! HgModule
IF (HG_CALC) THEN
  OPEN (CONHG, FILE="W2_Hg.csv",STATUS='OLD',IOSTAT=I)
  IF (I /= 0) THEN
    TEXT = 'Could not open W2_Hg.csv'
    GO TO 240
  END IF
  CALL HgModule_Input
  CALL HgModule_IC
  CALL HgKineticFluxesOutputInit
END IF
...
! Restart data
  ...
  IF (HG_CALC) THEN
    JDAYSTARTHG=JDAY;NXTMFLHG = JDAY + HGFLUXINT
  ENDIF
  ...
  IF (RESTART_IN) THEN
    ...
    ! HgModule
    IF(use_BedHg) READ(RSI) BedISS, Hg2    
  ...
  IF (PKSD_INPUT_WARNING) THEN        !SR 11/09/19

...
!!$OMP PARALLEL DO
  DO I=IU,ID
    ...
    DO K=KT,KB(I)
      ...
      DO1(K,I)          = O2(K,I)/(O2(K,I)+KDO(JW)) 
      DO2(K,I)          = 1.0 - DO1(K,I)                         !O2(K,I)/(O2(K,I)+KDO)
      DO3(K,I)          = (1.0+SIGN(1.0,O2(K,I)-1.E-10)) *0.5
      DO4(K,I)          = DO1(K,I) + ANOX(JW)*KDO(JW)/(O2(K,I)+KDO(JW))    !ZZ 2/22
      !DO4(K,I)          = MAX(DO1(K,I),ANOX(JW)*KDO(JW)/(O2(K,I)+KDO(JW)))   ! sw 10/27/2022
      ...
      IF(ORGC_CALC)THEN
        ...
        OMTRMDO3        =  OMTRM(K,I)*DO4(K,I)/ORGC(JW)
        ...
        LPOMHD(K,I)     =  OMTRM(K,I)*LPOMHK(JW)*LPOMC(K,I)      
        RPOMHD(K,I)     =  OMTRM(K,I)*RPOMHK(JW)*RPOMC(K,I)  
      ELSE
        !OMTRMDO3        =  OMTRM(K,I)*DO3(K,I)
        OMTRMDO3        =  OMTRM(K,I)*DO4(K,I)
        ...
        LPOMHD(K,I)     =  OMTRM(K,I)*LPOMHK(JW)*LPOM(K,I)             
        RPOMHD(K,I)     =  OMTRM(K,I)*RPOMHK(JW)*RPOM(K,I)  
      END IF
      !
      IF(CAC(NLDOMP) == '      ON')THEN
        ...
        LDOMPD(K,I)     =  OMTRM(K,I)  *LDOMPDK(JW) *LDOMP(K,I)*DO4(K,I)
        LRDOMPD(K,I)    =  OMTRM(K,I)  *LRDOMPDK(JW)*LDOMP(K,I)*DO4(K,I) 
      ELSE
      ...
      IF(CAC(NRDOMP) == '      ON')THEN
        ...
        RDOMPD(K,I)     =  OMTRM(K,I)  *RDOMPDK(JW) *RDOMP(K,I)*DO4(K,I)
      ELSE
      ...
      IF(CAC(NLPOMP) == '      ON')THEN
        ...
        LPOMPD(K,I)     =  OMTRM(K,I)  *LPOMPDK(JW) *LPOMP(K,I)*DO4(K,I)
        LRPOMPD(K,I)    =  OMTRM(K,I)  *LRPOMPDK(JW)*LPOMP(K,I)*DO4(K,I)
      ...
      IF(CAC(NRPOMP) == '      ON')THEN
        ...
        RPOMPD(K,I)     =  OMTRM(K,I)  *RPOMPDK(JW) *RPOMP(K,I)*DO4(K,I)
      ...
      IF(CAC(NLDOMN) == '      ON')THEN
        ...
        LDOMND(K,I)     =  OMTRM(K,I)  *LDOMNDK(JW) *LDOMN(K,I)*DO4(K,I)
        LRDOMND(K,I)    =  OMTRM(K,I)  *LRDOMNDK(JW)*LDOMN(K,I)*DO4(K,I)
      ELSE
      ...
      IF(CAC(NRDOMN) == '      ON')THEN
        ...
        RDOMND(K,I)     =  OMTRM(K,I)  *RDOMNDK(JW) *RDOMN(K,I)*DO4(K,I)
      ELSE
      ...
      IF(CAC(NLPOMN) == '      ON')THEN
        ...
        LPOMND(K,I)     =  OMTRM(K,I)  *LPOMNDK(JW) *LPOMN(K,I)*DO4(K,I)
        LRPOMND(K,I)    =  OMTRM(K,I)  *LRPOMNDK(JW)*LPOMN(K,I)*DO4(K,I)
      ...
      IF(CAC(NRPOMN) == '      ON')THEN
        ...
        RPOMND(K,I)     =  OMTRM(K,I)  *RPOMNDK(JW) *RPOMN(K,I)*DO4(K,I)
      ...
      IF(ORGC_CALC)THEN
        ...
        LPOMCHD(K,I)  =  OMTRM(K,I)*LPOMHK(JW)*LPOMC(K,I)  
        RPOMCHD(K,I)  =  OMTRM(K,I)*RPOMHK(JW)*RPOMC(K,I)  
      ELSE
